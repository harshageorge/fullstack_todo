...
function App() {
  ...
  const addTask = () => {
    if (newTask) {
      if (toDo.length !== 0) {
        ...
        axios.post('/todo/post', newEntry).then((response) => {}).catch((err)=>console.log(err));
      } else {
  ...
  const markDone = (id) => {
    ...
    axios.put(`/todo/edit/${id}`, filteredTasks[0]).then((response) => {
      console.log(response);
    }).catch((err)=>console.log(err));
  };
  ...
  const updateTask = () => {
    ...
    axios
      .put(`/todo/edit/${updateData.id}`, updateData)
      .then((response) => {
        console.log(response);
      }).catch((err)=>console.log(err));
    setUpdateData("");
